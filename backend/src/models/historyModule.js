...
export const getHistoryService = async(res,id)=>{
    ...
    const result = await pool.query("SELECT users.name,users.email,events.title from attendees JOIN users ON attendees.user_id = users.id JOIN events ON attendees.event_id = events.id where attendees.user_id=$1",[id]);

...
export const deletePostService = async(id) =>{
    const idExist = await pool.query("SELECT * FROM helpposts where id = $1",[id]);
    console.log("From Service PostModule: ",idExist);
    if(idExist.rowCount < 1){
        throw new Error("This ID is not exist in post. So Not Delete");
    }
    const result = await pool.query("DELETE FROM helpposts where id=$1",[id]);
    return result
}
